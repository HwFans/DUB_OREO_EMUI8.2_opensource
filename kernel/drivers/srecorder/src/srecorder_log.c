...
{
    struct device_node *srecorder_dts_node = NULL;
    const u32 *srecorder_dts_basep = NULL;

    /* get srecorder header address from dtsi */
    struct device_node *srecorder_header_node_dts_local = NULL;
    unsigned long srecorder_header_node_dts_addr_local = 0;
    unsigned long srecorder_header_node_dts_size_local = 0;
    srecorder_header_node_dts_local = of_find_compatible_node(NULL, NULL, "qcom,msm-imem-srecorder_header");
    if (srecorder_header_node_dts_local == 0) {
        SRECORDER_PRINTK("Finding compatible node msm-imem-srecorder_header failed.\n");
        return;
    }
    srecorder_dts_basep = of_get_address(srecorder_header_node_dts_local, 0, (u64*)&srecorder_header_node_dts_size_local, NULL);
    if (srecorder_dts_basep == 0) {
        SRECORDER_PRINTK("Getting dts srecorder_header address failed.\n");
        return;
    }
    srecorder_header_node_dts_addr_local = (unsigned long)of_translate_address(srecorder_header_node_dts_local, srecorder_dts_basep);
    if((srecorder_header_node_dts_addr_local == NULL) || (srecorder_header_node_dts_size_local!=CONFIG_SRECORDER_IMEM_HEADER_PHYS_SIZE))
    {
        SRECORDER_PRINTK("Getting srecorder_header address failed.\n");
        return;
    }
    SRECORDER_PRINTK("srecorder_header_address 0x%lx,size:0x%lx.\n",srecorder_header_node_dts_addr_local,srecorder_header_node_dts_size_local);
    srecorder_dts_basep == NULL;

    /* the high header in imem */
#ifdef CONFIG_ARM
    srecorder_imem_va = (unsigned long)ioremap_nocache(srecorder_header_node_dts_addr_local, srecorder_header_node_dts_size_local);
#else
    srecorder_imem_va = (unsigned long)ioremap_wc(srecorder_header_node_dts_addr_local, srecorder_header_node_dts_size_local);
#endif
	...
	/* dts node */
	srecorder_dts_node = of_find_compatible_node(NULL, NULL, "srecorder_mem");
	if (srecorder_dts_node == 0) {
        SRECORDER_PRINTK("Finding compatible node failed.\n");
        return;
	}

	/* buffer size */
	srecorder_dts_basep = of_get_address(srecorder_dts_node, 0, (u64*)&srecorder_dts_size, NULL);
	if (srecorder_dts_basep == 0) {
        SRECORDER_PRINTK("Getting address failed.\n");
        return;
	}

	/* buffer physical address */
	srecorder_dts_pa = (unsigned long)of_translate_address(srecorder_dts_node, srecorder_dts_basep);
...
#else
    /* of_iomap does not work for srecorder, we should use write combining here */
