...
void srecorder_sahara_clear(struct sahara_boot_log *head)
{
	...
	printk(KERN_ERR "head->sbl_log_addr:%lx\n",(long unsigned int)(head->sbl_log_addr));
	printk(KERN_ERR "head->lk_log_addr:%lx\n",(long unsigned int)(head->lk_log_addr));
	printk(KERN_ERR "head->sbl_log_size:%lx\n",(long unsigned int)(head->sbl_log_size));
	printk(KERN_ERR "head->lk_log_size:%lx\n",(long unsigned int)(head->lk_log_size));
	head->sbl_log_addr=0;
	head->lk_log_addr=0;
	head->sbl_log_size=0;
	head->lk_log_size=0;
...
void lk_log_write_to_kernel(struct work_struct *work)
{
    ...
    sbl1_lk_head=container_of(dw,struct workqueue_sbl1_lk_log,sbl1_lk_log_handle);
    ...
    printk(KERN_ERR "Bootloader log:i:%d len:%d\n",i,len);
...
uint32_t minidump_get_kramdom(void)
{
    ...
    printk(KERN_ERR "Minidump get random number: %u\n", dumpid_ramdom);
...
void srecorder_save_kernel_log_addr()
{
...
#endif
	phys_addr_t kernel_log_check_addr_number = 0;
	...
	kernel_log_check_addr_number = virt_to_phys((void *)kallsyms_lookup_name("__log_buf"));
	/* boot_log->kernel_log_addr is uint32,we must sure that it is not overflow */
	if(kernel_log_check_addr_number >= MINIDUMP_UINT32_MAX_NUM)
	{
		boot_log->kernel_log_addr = 0;
		SRECORDER_PRINTK("Kernel log buffer address overflow\n");
	}
	else
	{
		boot_log->kernel_log_addr = kernel_log_check_addr_number;
	}
#else
    boot_log->kernel_log_addr = 0;
#endif
    ...
    if((workqueue_get_log.size)>=(SAHARA_BOOT_LOG_HALF_SIZE-sizeof(struct sahara_boot_log)))
    {
        SRECORDER_PRINTK("minidump:uart size error\n");
        workqueue_get_log.size = 0;
    }
...
//	lk_log_write_to_kernel2((char *)(boot_log2+sizeof(struct sahara_boot_log)),(unsigned int)(boot_log2->lk_log_size+boot_log2->sbl_log_size));
    ...
    SRECORDER_PRINTK("srecorder_sahara sbl1_log_addr:%lx sbl1_log_size:%lx \n",(long unsigned int)(boot_log->sbl_log_addr),(long unsigned int)(boot_log->sbl_log_size));
    SRECORDER_PRINTK("srecorder_sahara lk_log_addr:%lx lk_log_size:%lx \n",(long unsigned int)(boot_log->lk_log_addr),(long unsigned int)(boot_log->lk_log_size));
    SRECORDER_PRINTK("srecorder_sahara kernel_log_addr:%lx kernel_log_size:%lx \n",(long unsigned int)(boot_log->kernel_log_addr),(long unsigned int)(boot_log->kernel_log_size));
